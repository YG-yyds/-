...
图1: VULNUS处理包含122个节点、22个子网络和846个漏洞的真实网络。使用修改的树状图条形图表示，子网络水平排列。每个条表示子网络(n1, n2, …)，显示其节点；节点大小和条长度都与元素的漏洞总数成比例。条形图按长度排序，当用户滚动到主窗格(A)时，窗格(B)保持视觉上下文。每个条形图(a)的最左侧部分包含具有零漏洞的节点，这些节点表示为固定维度的正方形。鼠标悬停在节点(b)上可显示其CVSS分数和漏洞数量。节点可以单独选择，也可以通过主窗格中的子网络选择，并在选择窗格(D)中按其分数的统一和统计区间进行选择。所选节点列在(C)中，它们的漏洞呈现在频率图(E)上，并可通过分数(F)进行过滤。(I)中列出了频率图上的进一步选择，允许比较雷达图上的漏洞得分(G)或检查数值得分并访问CVE参考页面(H)。使用攻击图信息自动计算近似的最佳修复策略(J)，VULNUS允许安全管理器交互模拟漏洞修复，以探索修复计划的次优变体。

#### 全局可视化

最具挑战性的要求之一是为用户提供一个有效的漏洞分布、核心程度和严重程度的概览。

主窗格(图1.A)代表系统的重点：网络通过修改过的树状图条形图表示，每个条形图都包含属于该分组的元素，或者是子网络、节点，或者是漏洞。在高层次上，条形图的组织结构允许显示与分组的总规模有关的优先视图：图1.A中，分组是子网络，元素是节点，其规模编码为每个节点上的漏洞数量。系统对漏洞数量最多的子网络进行优先排序(自上而下)，节点从左到右按漏洞数量递增排序。

#### 节点表示

漏洞数量、基础得分、可利用性得分和目标环境得分是每个节点上最相关的视觉编码指标，大小分五个区间，增加视觉属性值之间的分离，视觉编码的最终版本如下：

- 其大小编码了漏洞的数量
- 基础得分平均值用一个五位的滑动窗口表示(图2.a)，从底部(0)到顶部(10)，逐渐覆盖蓝色背景
- 目标环境得分的最大值被编码在滑动窗口的颜色上，有一个五值的红色刻度(图2.b)，强红色对应于最高和更危险的区间
- 漏洞可利用性得分的分布用水平细条图来编码(图2.c)，五级灰度代表五个可利用性得分区间(黑色=容易利用，白色=难以利用)
- 节点左上方三角形(图2.d)表示该节点是目标节点(黄色)或源节点(浅蓝色)

<img src="pic\p4\fig2.png" width="25%" />

图2: 编码节点类型和节点31上的分数。a. 基础分数由红色滑动窗口编码，该窗口逐渐覆盖节点蓝色背景，处于第四级([6，8]); b) 目标环境分数由红色滑动窗口编码，处于第四级([6，8]); c. 可利用性柱状图显示，大约25%的漏洞具有非常高的可利用性([8，10]); d. 黄色三角形将节点归类为目标节点；e. 粉红色向上的箭头表示目标环境相对于上次扫描有所增加。

#### 分析工具

VULNUS提供不同分析模式，这些模式依赖于两个步骤的基本迭代：

- 根据不同的角度逐步选择有趣的元素(R2a)
- 检查和过滤属于所选元素的漏洞的相关频率分布(R2b)

VULNUS提供不同选择元素的方法：手动选择、分组选择、类型选择、度量区间选择。筛选操作通过交互式图例进行，显示用于三个指标(基础、可利用性、目标环境得分)的视觉编码，可点击每个指标的一个或多个区间，每个指标都有一个交互式箱形图显示其在网络所有元素中数值分布。

安全管理员筛选并分析主动选择的漏洞的频率分布(图1.E)，通过点击相关条形图选择一个漏洞将其加入到雷达图上突出显示的所选漏洞列表中(图1.I)，点击列表一个元素，在所有共享该漏洞的节点显示一个彩色条(图3)，可以轻松比较多个漏洞。

<img src="pic\p4\fig3.png" width="50%" />

图3: 网络上传播的漏洞详情。从频率分布中选择了六个漏洞，查找受其中四个漏洞影响的节点。

若选择漏洞数量过多，系统提供一个过滤机制(图1.F)，将约束指标分为5个区间并将其分布显示在箱形图。

可以切换环境(图1.C)并在主窗格中查看不同的网络表示(图4)。

<img src="pic\p4\fig4.png" width="100%" />

图4:  a) 子网络环境，每个子网络都有一个条形图，其中元素代表其节点; b) 打分环境，元素代表节点，按基本分数的间隔分组的; c) 网络环境，网络被表示为一个条形图，其中元素代表子网络; d) 漏洞环境，漏洞显示为单条的元素，每个元素的大小与网络中共享该漏洞的节点数成正比。

#### 假设分析

系统支持确定修复策略(R3)，计算AOF策略并在what-if窗格(图1.J)提出，按从消除最高数量攻击路径的策略到消除最小数量攻击路径的策略排序。系统实现一个假设分析，通过点击要修复的漏洞实例旁边的切换键来模拟修复的应用：环境更新，显示应用修复后可能获得的状态。更新主窗格中的元素，删除将真实状态与上一次扫描进行比较的指示器，并添加闪烁的绿色向下箭头，以指示模拟中节点的目标环境降低。通过这种方式，安全管理员可以将近似的最优修复策略AOF（每次模拟修复时都会重新计算）与关于修复的适用性的知识相匹配，手动识别一个次优的AOF并估计效果。

图5显示与实施VULNUS之前使用的策略相比，最佳修复策略减少中断所有攻击路径所需的修复数量的程度。

<img src="pic\p4\fig5.png" width="40%" />

图5: 蓝色: 使用拟议算法计算的最佳修复策略; 橙色: 根据CVSS基本得分对修复进行排序的策略

## 评价

进行定量评估，从有效性(用户实现特定目标的准确性和完整性)和效率(与有效性相关的努力)方面描述当前界面的可用性。

<img src="pic\p4\fig6.png" width="100%" />

图6: VULNUS系统评估结果。a) 箱形图显示20名参与者的平均分数分布; b) 箱形图描述了平均总完成时间分布(分钟); c) 条形图显示了每项任务的正确答案汇总比例; d) 条形图说明每个问题正确答案的比例

<img src="pic\p4\table1.png" width="40%" />

表1: 用户对环境的投票，体现四个环境的有用性和使用的简单性和在三个主要活动方面的能力

<img src="pic\p4\table2.png" width="50%" />

表2: 三个活动的平均信息量(从0=完全没有信息量，到10=完全有信息量)

## 总结

介绍用于分析计算机网络漏洞的VULNUS的解决方案，提供了分布在网络节点上大量漏洞的概述和细节，突出特点是与CVSS和攻击图数据紧密联系，提出对CVSS环境评分的修改，允许自动计算近似的最佳修复策略，减少网络上的干预次数。同时，系统证实以用户为中心的设计开发的有用性和有效性。

三点考虑：

- 关于标准可视化的使用，通常代表用户请求的第一个设计解决方案：有时不能完全满足用户需求，准备好采用某种非常规但更有效的解决方案是一个好主意。
- 在复杂的网络场景中，自动化的解决方案即使是最优的，也可能不适用于分析环境之外的问题(比如商业约束)，即使已知最佳解决方案，探索替代策略的模拟场景也是有用的。
- 过滤时，用户在修改当前的选择时遇到困难，因为不容易将其与过滤动作联系起来

未来方向：

- 执行完整的跟踪分析
- 探索新的修补策略
- 挑战所提出的视觉编码的通用性并在不同领域的多变量数据上测试
